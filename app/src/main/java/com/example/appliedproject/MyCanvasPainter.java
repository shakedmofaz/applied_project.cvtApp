...
import static com.example.appliedproject.firstFragment.*;
...
public class MyCanvasPainter {
    ...
    private MyCanvasPainter(Context context) {
        this.context = context;

        isAutoModeEnabled = true;
    }

    static MyCanvasPainter with(Context context) {
        return new MyCanvasPainter(context);
    }

    MyCanvasPainter paint(MyPitchDifference pitchDifference) {
    ...
    void on(Canvas canvas) {
        ...
        if (!isAutoModeEnabled) {
            MyNote[] tuningNotes = getCurrentTuning().getNotes();
            MyNote note = tuningNotes[getReferencePosition()];
    ...
    private void drawListeningIndicator() {
        ...
        if (SubTunerFragment.IS_RECORDING) {
    ...
    private void drawText(float x, float y, MyNote note, Paint textPaint) {
    ...
    private String getNote(MyNoteName name) {
        if (useScientificNotation) {
